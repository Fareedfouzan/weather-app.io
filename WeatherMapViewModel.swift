    ...
    func getCoordinatesForCity() async throws {
        ...
        if let placemarks = try? await geocoder.geocodeAddressString(city)
         ,let location = placemarks.first?.location?.coordinate {
                ...
                print("Error: No coordinates found in placemarks")
            }
        }
    
    
    func URLStringForIcon(for iconCode: String) -> String {
        return "https://openweathermap.org/img/wn/\(iconCode)@2x.png"
    }
    
    func loadData(lat: Double, lon: Double) async throws -> WeatherDataModel {
        // MARK:  add your appid in the url below:
        if let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&units=metric&appid=1eedd64d55a52d698f0ce8ffd6ce9cce") {
